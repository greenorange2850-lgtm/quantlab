...
export async function runAdaptiveSearch(
  ...
  const { config, candles, onProgress, signal, pauseController } = options
  ...
  const emit = (status: RandomSearchLiveStatus) => {
    ...
    session.progress = {
      ...
      stage: tracker.stage,
      stageBudgets: { ...tracker.stageBudgets, exploration: { ...tracker.stageBudgets.exploration }, refinement: { ...tracker.stageBudgets.refinement }, stability: { ...tracker.stageBudgets.stability }, baseline: { ...tracker.stageBudgets.baseline } },
  ...
  async function handlePause(): Promise<boolean> {
    if (!pauseController?.paused) return false
    emit('PAUSING')
